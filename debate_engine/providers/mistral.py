...
class MistralProvider(BaseProvider):
    """Provedor para API Mistral AI com retry e fallback de modelo."""

    def __init__(self, api_key: str, modelos: List[str] = None, rate_limit: int = 5):
        super().__init__(nome="mistral", modelos=modelos or ["mistral-small-latest"], rate_limit=rate_limit)
            ...
            for tentativa in range(3):
                try:
                    ...
                    if "429" in resultado or "503" in resultado:
                        ...
                        print(f"⏳ Mistral: {resultado[:40]}, aguardando {espera}s (tentativa {tentativa+1}/3)...")
                        time.sleep(espera)
                        continue
                    else:
                        return resultado
                except Exception as e:
                    print(f"⚠️ Mistral: erro inesperado: {str(e)[:60]}")
                    if tentativa < 2:
                        time.sleep(5)
                    continue
            print(f"⚠️ Mistral ({modelo}): falhou após 3 tentativas, tentando próximo modelo...")
